...
(function() {
    ...
    ThreeHeightMap.prototype.display = function(options, filters){
        options = this.merge('three', options);
    ...
    ThreeHeightMap.prototype.setObjects = function(filters){
        ...
        function getCentre(grid, filters) {
            return {
        ...
        var centre = getCentre(this.grid, filters);
        ...
        while (row--) {
            ...
            while (col--) {
                ...
                switch(filters.geometry){
                    case 'Cylinder':
                        geometry = new THREE.CylinderGeometry(0.5, 0.5, height + filters.baseHeight, 20);
                    break;
                    case 'Pyramid':
                        geometry = new THREE.CylinderGeometry(0, 0.5, height + filters.baseHeight, 4);
                    break;
                    case 'Prism':
                        geometry = new THREE.BoxGeometry(1, height + filters.baseHeight, 1);
                    break;
                    default:
                       geometry = new THREE.BoxGeometry(1, height + filters.baseHeight, 1);
                }
    ...
    ThreeHeightMap.prototype.setLights = function(options){
        var light;
        options = options || {};
        options.hex = options.hex || 0xffffff;
        options.hex = options.intensity || 0.5;
        
        light = new THREE.DirectionalLight(options.hex, options.intensity);
    ...
    window.ThreeHeightMap = ThreeHeightMap;
    
}(window, ImageHeightMap, THREE));
